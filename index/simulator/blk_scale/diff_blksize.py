...
def genworkload(parquetdir, column, logname):
    ...
    _ranges = workload.genseries_range(0.00001, 200)
    wf = open(logname, "w+", encoding="UTF-8")
    wf.write("selectivity 1e-05\n")
    for _range in _ranges:
        wf.write("range:{},{}\n".format(_range[0], _range[1]))
    wf.write("selectivity point\n")
...
if __name__=="__main__":
    ...
    genworkload("../../../dataset/diffblock/lon/recordperblock50000/", column, logname)
    for v in [1e4, 5e4, 1e5, 2e5]:
        recordperblock = int(v)
        writedir = "../../../dataset/diffblock/{}/recordperblock{}/".format(column, recordperblock)
        compare(writedir, column, recordperblock, error, logname)
